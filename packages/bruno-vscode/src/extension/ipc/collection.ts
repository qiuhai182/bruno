...
import { readTextFile, readTextFileSync } from '../utils/encoding';
...
const registerCollectionIpc = (watcher: CollectionWatcherInterface): void => {
    ...
    try {
      ...
      if (isDirectory(oldPath)) {
        ...
        if (fs.existsSync(folderFilePath)) {
          const oldContent = readTextFileSync(folderFilePath).data;
        ...
        const oldContent = readTextFileSync(oldPath).data;
        ...
        for (const child of (sourceItem.items || [])) {
          if (child.type === 'folder') {
            ...
            if (sourceFormat === format) {
              ...
              const childContent = readTextFileSync(child.pathname).data;
  ...
  registerHandler('renderer:clone-request', async (args) => {
    ...
    try {
      ...
      const content = readTextFileSync(sourcePathname).data;
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
              ...
              try {
                const content = readTextFileSync(entryPath).data;
    ...
    for (let i = 0; i < requestFiles.length; i += PARSE_PARALLELISM) {
      ...
      await Promise.all(
        chunk.map(async (filePath) => {
          try {
            const [fileStats, { data: content }] = await Promise.all([
