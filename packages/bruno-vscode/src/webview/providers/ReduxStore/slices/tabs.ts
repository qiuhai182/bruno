...
const tabTypeAlreadyExists = (tabs: Tab[], collectionUid: string, type: string) => {
...
export const tabsSlice = createSlice({
  ...
  reducers: {
    ...
    switchTab: (state, action: PayloadAction<any>) => {
    ...
    updateRequestPaneTabWidth: (state, action: PayloadAction<any>) => {
    ...
    updateRequestPaneTabHeight: (state, action: PayloadAction<any>) => {
      const tab = find(state.tabs, (t) => t.uid === action.payload.uid);
    ...
    updateRequestPaneTab: (state, action: PayloadAction<any>) => {
      const tab = find(state.tabs, (t) => t.uid === action.payload.uid);
    ...
    updateResponsePaneScrollPosition: (state, action: PayloadAction<any>) => {
      const tab = find(state.tabs, (t) => t.uid === action.payload.uid);
    ...
    updateResponseFormat: (state, action: PayloadAction<any>) => {
      const tab = find(state.tabs, (t) => t.uid === action.payload.uid);
    ...
    updateResponseViewTab: (state, action: PayloadAction<any>) => {
      const tab = find(state.tabs, (t) => t.uid === action.payload.uid);
    ...
    closeAllCollectionTabs: (state, action: PayloadAction<any>) => {
      const collectionUid = action.payload.collectionUid;
      state.tabs = filter(state.tabs, (t) => t.collectionUid !== collectionUid);
      state.activeTabUid = null;
    },
    makeTabPermanent: (state, action: PayloadAction<any>) => {
