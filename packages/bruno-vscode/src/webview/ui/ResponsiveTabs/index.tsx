...
interface DROPDOWN_WIDTHProps {
  tabs: unknown[];
  activeTab?: boolean;
  onTabSelect?: (...args: unknown[]) => void;
  rightContent?: React.ReactNode;
  rightContentRef: unknown;
  delayedTabs?: unknown;
...
const areTabArraysEqual = (a: any, b: any) => {
  if (a.length !== b.length) return false;
  return a.every((tab: any, index: any) => tab.key === b[index].key);
};
...
const ResponsiveTabs = ({
  ...
  // Optional: width of the expandable element when expanded
  rightContentExpandedWidth,

  // Optional: index of the expandable child element (-1 means last child)
  expandableElementIndex = -1
}: any) => {
  ...
  const tabRefsMap = useRef<Record<string, HTMLElement>>({});
  ...
  const calculateTabVisibility = useCallback(() => {
    ...
    if (!visible.some((t: { key: string }) => t.key === activeTab) && overflow.length) {
      const activeTabIndex = overflow.findIndex((t: { key: string }) => t.key === activeTab);
      if (activeTabIndex !== -1) {
  ...
  useEffect(() => {
    const currentKeys = new Set(tabs.map((t: any) => t.key));
  ...
  const setTabRef = useCallback((el: any, key: any) => {
    if (el) {
  ...
  const renderTab = (tab: any) => {
    ...
    // Always get the latest tab data from the tabs prop to ensure indicators are fresh
    const currentTab = tabs.find((t: any) => t.key === tab.key) || tab;
  ...
  const overflowMenuItems = useMemo(() => {
    return overflowTabs.map((tab) => {
      ...
      return {
        ...
        ariaLabel: typeof currentTab.label === 'string' ? currentTab.label : tab.key,
        onClick: () => handleTabSelect(tab.key),
        className: classnames({ active: tab.key === activeTab })
      };
    });
  }, [overflowTabs, tabs, activeTab, handleTabSelect]);
