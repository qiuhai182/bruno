...
const EnvironmentVariables = ({
  ...
  const { globalEnvironments, activeGlobalEnvironmentUid } = useSelector((state: any) => state.globalEnvironments);

  const globalEnvironmentDraft = useSelector((state: any) => state.globalEnvironments?.globalEnvironmentDraft);
  const environmentsDraft = isGlobal ? globalEnvironmentDraft : collection?.environmentsDraft;
  ...
  const nonSecretSensitiveVarUsageMap = useMemo(() => {
    ...
    const nonSecretVars = environment.variables.filter((v: any) => v.enabled && !v.secret && v.name);
    if (!nonSecretVars.length) {
      return result;
    }
    const varNames = new Set(nonSecretVars.map((v: any) => v.name));

    const checkSensitiveField = (obj: any, fieldPath: any) => {
    ...
    const getObjectToProcess = (item: any) => {
  ...
  const hasSensitiveUsage = (name: any) => !!nonSecretSensitiveVarUsageMap[name];
  ...
  const handleNameChange = (index: any, e: any) => {
    ...
    if (isLastRow) {
      ...
      setTimeout(() => {
        formik.setFieldValue(String(formik.values.length), newVariable, false);
      }, 0);
    }
  };

  const handleNameBlur = (index: any) => {
    formik.setFieldTouched(`${index}.name`, true, true);
  };

  const handleNameKeyDown = (index: any, e: any) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      formik.setFieldTouched(`${index}.name`, true, true);
    }
  ...
  const handleSave = () => {
    ...
    const saveAction = isGlobal
      ? saveGlobalEnvironment({ variables: cloneDeep(variablesToSave), environmentUid: environment.uid })
      : saveEnvironment(cloneDeep(variablesToSave), environment.uid, collection.uid);

    (dispatch(saveAction) as unknown as Promise<void>)
      .then(() => {
