...
const FolderSettings = ({
  ...
  const folderRoot = folder?.draft?.root || folder?.root;
  ...
  const activeHeadersCount = headers.filter((header: any) => header.enabled).length;

  const requestVars = folderRoot?.request?.vars?.req || [];
  const responseVars = folderRoot?.request?.vars?.res || [];
  const activeVarsCount = requestVars.filter((v: any) => v.enabled).length + responseVars.filter((v: any) => v.enabled).length;
