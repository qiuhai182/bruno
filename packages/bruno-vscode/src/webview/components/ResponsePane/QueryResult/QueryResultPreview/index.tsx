...
interface QueryResultPreviewProps {
  ...
  codeMirrorMode?: React.ReactNode;
  previewMode?: unknown;
  disableRunEventListener?: boolean;
  displayedTheme?: boolean;
  numPages?: React.ReactNode;
}
...
const QueryResultPreview = ({
  ...
  if (selectedTab === 'editor') {
    return (
      <CodeEditor
        ...
        value={formattedData}
        mode={codeMirrorMode}
        initialScroll={focusedTab && focusedTab.responsePaneScrollPosition != null ? focusedTab.responsePaneScrollPosition : 0}
        readOnly
      />
    );
  }
  ...
  switch (previewMode) {
    ...
    case 'preview-audio': {
      return <audio controls src={`data:${contentType.replace(/\;(.*)/, '')};base64,${dataBuffer}`} className="mx-auto" />;
    }
    case 'preview-video': {
