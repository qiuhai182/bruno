...
const bypassPnPPlugin = {
  ...
  setup(build) {
    ...
    build.onResolve({ filter: /.*/ }, (args) => {
      ...
      // Shared encoding core lives in the monorepo's bruno-filestore package
      // (single source of truth, also used by the Electron app)
      if (args.path === '@bruno-encoding') {
        return { path: path.join(__dirname, '..', 'bruno-filestore', 'src', 'utils', 'encoding.ts') };
      }
